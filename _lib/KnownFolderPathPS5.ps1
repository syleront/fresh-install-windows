...
# Function to grant permissions to the current user on a path
function Grant-Permissions($path) {
  ...
  Write-Output "Permissions granted to current user for $path."
}

function New-DirectoryForUser {
  Param (
    [Parameter(Mandatory = $true)]
    [string]$path
  )
  ...
  foreach ($directory in $directories) {
    if ($currentPath -eq "") {
      ...
      $currentPath = Join-Path $currentPath $directory # [System.IO.Path]::Combine($currentPath, $directory)
    }
    if (-not (Test-Path $currentPath)) {
      # Create the directory
      New-Item -Path $currentPath -ItemType Directory -Force > $null
      # Grant permissions to the current user
      Grant-Permissions $currentPath
    }
  }
}

function Set-KnownFolderPathWrapper {
  Param (
    [Parameter(Mandatory = $true)]
    [string]$KnownFolder,
    [Parameter(Mandatory = $true)]
  ...
  if (-Not (Test-Path -Path $Path)) {
    New-DirectoryForUser -path $path
  }

  $old = ""
  $new = ""
  Get-KnownFolderPath $KnownFolder ([ref]$old) > $null
  Set-KnownFolderPath $KnownFolder $Path > $null
  Get-KnownFolderPath $KnownFolder ([ref]$new) > $null
  Write-Output "Set library $old -> $new"
}
